...
def upload_words_format(message):
    data = db.output_words(message.chat.id)  # Expecting list of tuples: (foreign, native, group)
...
def final_show(message):
    ...
    data = db.output_words(message.chat.id, show_cache[message.chat.id]["groups"], show_cache[message.chat.id]["langs"])
    msg_text = "Your words:\n"
    for line in data:
...
def edit_words(message):
    msg = bot.reply_to(message, "Write the word index from /show that you want to edit")
    bot.register_next_step_handler(msg, select_edit_option)


def select_edit_option(message):
    try:
        index = int(message.text)
        edit_cache[message.chat.id] = show_cache[message.chat.id][index]
    except (TypeError, KeyError, ValueError):
        msg = bot.reply_to(message, 'Invalid index. Please try again.')
...
def enter_en_change(message):
    db.change_word(message.chat.id, {"lang": "en", "changeable": edit_cache[message.chat.id][0], "changed": message.text})
...
def callback_query(call):
    if call.data == 'edit_cb_del':
        ...
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(InlineKeyboardButton('Change Foreign', callback_data='edit_change_en'),
                   InlineKeyboardButton('Change Native', callback_data='edit_change_ru'))
        bot.edit_message_text(f"Choose which part to change:\n{edit_cache[call.message.chat.id]}", call.message.chat.id, call.message.message_id, reply_markup=markup)
    elif call.data == "edit_change_en":
        bot.edit_message_text("Enter new foreign word:", call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler(call.message, enter_en_change)
    elif call.data == "edit_change_ru":
        bot.edit_message_text("Enter new native word:", call.message.chat.id, call.message.message_id)
